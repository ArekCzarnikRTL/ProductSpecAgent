# Frontend: Design System Spezifikation
...
:root {
  ...
  --chart-4: oklch(0.7 0.15 60);    /* Gelb-Gruen */
...
Dark Mode ist der **einzige Modus** (kein Light Mode). Aktivierung ueber CSS-Klasse auf `<html>`:
...
| Ueberschriften | Inter | `--font-heading` |
...
| `text-base` | 1rem (16px) | Ueberschriften, wichtiger Text |
| `text-lg` | 1.125rem (18px) | Grosse Ueberschriften |
...
| `font-medium` | 500 | Labels, Ueberschriften |
| `font-semibold` | 600 | Betonte Ueberschriften |
| `font-bold` | 700 | Starke Betonung |

### Zeilenhoehen

- `leading-snug` (1.375) -- Ueberschriften
...
| `gap-4` | 16px | Grosser Abstand |
| `gap-6` | 24px | Sektions-Abstand |

### Hoehen-Standards

| Element | Hoehe | Tailwind |
|---------|-------|----------|
| Button xs | 24px | `h-6` |
| Button sm | 28px | `h-7` |
...
Hoehe: `h-5`, abgerundet: `rounded-4xl`.
...
Icons erben die Textfarbe (`text-current`) oder werden explizit eingefaerbt:

```tsx
<Icon className="size-4 text-muted-foreground" />   // Gedaempft
<Icon className="size-4 text-primary" />             // Primaer
...
Das Layout ist primaer fuer Desktop optimiert (feste Sidebar-Breiten).

---

## CVA Component Pattern

Standardmuster fuer alle Komponenten:
...
### SpecFlowGraph (Rete.js)

Der Node-Graph fuer den Idea-to-Spec Flow. Nutzt das Design-System fuer Node-Styling:

| Node-Status | Farbe | Badge |
|-------------|-------|-------|
| Offen | `--muted` | `ghost` Badge |
| In Arbeit | `--primary` | `default` Badge |
| Fertig | `--chart-2` (Teal) | `secondary` Badge |
| Clarification | `--chart-5` (Orange-Rot) | `destructive` Badge |
| Decision | `--chart-3` (Violett) | `outline` Badge |

Node-Connections nutzen `--border` Farbe, aktive Connections `--primary`.

### ProjectCard

Dashboard-Karte fuer Projekte. Basiert auf `Card` Component:
...
<Card>
  <CardHeader>
    ...
    <CardAction><Badge>{status}</Badge></CardAction>
  </CardHeader>
  <CardContent>
    <ProgressBar value={specProgress} />
    <div className="flex gap-2">
      <Badge variant="outline">{openDecisions} Decisions</Badge>
      <Badge variant="outline">{openClarifications} Clarifications</Badge>
    </div>
  </CardContent>
</Card>
```
...
Entscheidungskarte mit Optionen. Basiert auf `Card` + `Tabs`:
    ...
    <Tabs>
      {options.map(option => (
        <TabsContent>
          ...
          <div className="text-chart-2">Pro: {option.pros}</div>
          <div className="text-destructive">Contra: {option.cons}</div>
        </TabsContent>
      ))}
    </Tabs>
    <Alert>
      <AlertTitle>AI-Empfehlung</AlertTitle>
